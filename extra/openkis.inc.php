...
function openkis_UpdateCoords($mod,$forceall=false)
{
    ...
    if ($forceall)
        ...
        $results=FN_XMLQuery("select {$params['fields']} FROM {$config['tables']} WHERE recordupdate > coordnatesupdated OR latitude LIKE '0'");
...
function openkis_GetItemPosition($row)
{
    ...
    $ret['elevation']=str_replace(",",".",round($elevation,7));
    return $ret;
}
